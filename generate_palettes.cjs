...
const palettes = [];
// Generate 150 palettes (150 * 4 = 600 colors)
for (let i = 1; i <= 150; i++) {
  palettes.push(generatePalette(i));
}

const fileContent = `export const galleryItems = ${JSON.stringify(palettes, null, 2)};\n`;
fs.mkdirSync('src/data', { recursive: true });
fs.writeFileSync('src/data/palettes.ts', fileContent);
